...
const { data: events } = await useListEventUsers({
    ...
    >
      ...
      >
        <div class="text-lg pt-6">
          ...
          <span class="font-bold text-violet-950"
            >Mis Ceremonias y Retiros</span
          >
          <p>
            Conoce las próximas Ceremonias y Retiros en los que estas inscrito.
          </p>
        </div>
      </div>
      <div v-if="events?.data.length == 0" class="p-4 bg-blue-100 mx-5 my-6">
        <p>Aun no te has registrado a ningun evento</p>
      </div>
      <div
        v-for="(event, index) in events?.data"
        :key="index"
        class="px-5 py-6 flex flex-col"
      >
        <a :href="'/event/' + event.events?.id" class="card py-4 w-full">
          <div class="h-60 w-full">
            <img
              class="w-full h-full rounded-lg object-fit"
              :src="'https://d334a63s5wk7yh.cloudfront.net/'+ event.events?.photos![0]"
            />
          </div>
          <div class="grid pt-2">
            <span class="font-bold">{{ event.events?.name }}</span>
            <span>{{
              format(
                new Date(event.events?.datetime_start || ""),
