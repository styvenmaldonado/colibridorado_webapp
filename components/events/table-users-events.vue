    ...
    <template v-slot:item.users="{ value }">
      <div class="grid py-3">
        <span class="font-bold uppercase"
          >{{ value.family_name }} {{ value.given_name }}</span
        >
    ...
    <template v-slot:item.medicalPreincription="{ value }">
      <div class="flex flex-col">
        <span
          ...
          >{{ JSON.parse(value).medicine }}</span
        >
        <span
          ><span class="font-bold">Referido por: </span>
          {{ JSON.parse(value).referBy }}</span
        >
        <span
          ><span class="font-bold">Condiciones Medicas: </span
          >{{ JSON.parse(value).preinscriptions }}</span
        >
      </div>
    </template>
  </v-data-table-server>
